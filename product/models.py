...
class ProductsManager(models.Manager):
    ...
    def get_related_product(self, product):
        return (
            self.get_queryset()
            .filter(
                category__in=Category.objects.get(id=product.id).get_descendants(
                    include_self=True
                ),
                status=True,
            )
...
class Product(models.Model):
    ...
    details = RichTextField(verbose_name="جزئيات محصول")
    analyze = RichTextField(blank=True, null=True, verbose_name="نقد و بررسی")
    status = models.CharField(max_length=50, choices=STATUS, verbose_name="وضعیت")
    slug = models.SlugField(
        verbose_name="عبارت لینک",
        null=False,
        unique=True,
        allow_unicode=True,
        max_length=200,

...
from account.signals import object_viewed_signal
...
from .serializers import (
    ...
    ProductFavouriteUpdateSerializer,
)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.request.user.is_authenticated:
            object_viewed_signal.send(
                product.__class__, instance=product, request=request
            )
...
class CategoryRetrieveAPIView(generics.RetrieveAPIView):
    ...
    def get(self, request, **kwargs):
        ...
        products = Product.objects.filter(
            category__in=Category.objects.get(title=cat_title).get_descendants(
                include_self=True
            )
        )
        filter_products = ProductFilter(self.request.GET, queryset=products)
        serializer = ProductSerializer(
            filter_products.qs, many=True, context={"request": request}
        )
...
class BrandDetailAPIViewSet(generics.RetrieveAPIView):
    ...
    def get(self, request, **kwargs):
        ...
        products = Product.objects.filter(
            brand__in=Brand.objects.filter(title=brand_title)
        )
        filter_products = ProductFilter(self.request.GET, queryset=products)
        serializer = ProductSerializer(
            filter_products.qs, many=True, context={"request": request}

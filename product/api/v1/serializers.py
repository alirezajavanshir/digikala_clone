...
class ProductSerializerLink(serializers.ModelSerializer):
    product_url = serializers.HyperlinkedIdentityField(view_name="products_detail_api")
    variation_set = VariationSerializerLink(
        source="variation", many=True, read_only=True
    )
...
class ProductDetailSerializer(serializers.ModelSerializer):
    ...
    comment_set = CommentsSerializerLink(
        source="comment",
        many=True,
        read_only=True,
    )
    gallery_set = ImagesSerializerLink(
        source="gallery",
        many=True,
        read_only=True,
    )
    attrs_set = AttrProductSerializerLink(
        source="attrs",
        many=True,
        read_only=True,
    )
...
class BrandDetailSerializer(serializers.ModelSerializer):
    product_set = ProductSerializerLink(
        many=True,
        read_only=True,
    )

...
class Comment(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, verbose_name="کاربر", related_name="comments"
    )
    ...
    advice = models.CharField(
        max_length=100, choices=ADVICE, default="omm", verbose_name="توصیه"
    )
    affective = models.ManyToManyField(
        User, blank=True, verbose_name="کامنت مفید", related_name="affective_comments"
    )
    affective_count = models.BigIntegerField(default=0, verbose_name="تعداد کامنت مفید")
    notaffective = models.ManyToManyField(
        User,
        blank=True,
        verbose_name="کامنت غیرمفید",
        related_name="notaffective_comments",
    )
    notaffective_count = models.BigIntegerField(
        default=0, verbose_name="تعداد کامنت غیرمفید"
    )
    ip = models.CharField(max_length=20, blank=True, verbose_name="آی‌پی")
    status = models.CharField(
        max_length=10, choices=STATUS, default="Submit", verbose_name="وضعیت"
    )
...
class RateComment(models.Model):
    attribute = models.ForeignKey(
        AttrProduct, on_delete=models.CASCADE, verbose_name="ویژگی"
    )

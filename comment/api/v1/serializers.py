...
class CommentSerializer(serializers.ModelSerializer):
    ...
    order_product = serializers.CharField(
        source="order_product.productt", required=False
    )
    ...
    def create(self, validated_data):
        ...
        new_comment = Comment.objects.create(
            user_id=user_data, product_id=product.id, **validated_data
        )
        ...
        if OrderProduct.objects.filter(
            user_id=user_data, product_id=product.id
        ).exists():
            new_comment.order_product = OrderProduct.objects.filter(
                user_id=user_data, product_id=product.id
            ).first()
        new_comment.save()
        return new_comment

...
from .serializers import (
    ShopCartListSerializer,
    ShopCartUpdateSerializer,
    ShopCartAddSerializer,
    OrderCreatDetailSerializer,
    OrderDetailSerializer1,
)
from order.models import ShopCart, Order, PostWay, OrderProduct
...
class ShopCartListAPIViewSet(generics.ListAPIView):
    ...
    def list(self, request):
        ...
        serializer = ShopCartListSerializer(
        ...
        if serializer.is_valid():
            ...
            if product.variant != "None":
                ...
                if variant.quantity >= quantity_data:
                    ...
                    return Response(
                        data=f"تعداد شفارش باید کمتر از {variant.quantity} باشد ",
                ...
                if product.amount >= quantity_data:
                    ...
                    return Response(
                        data=f"تعداد شفارش باید کمتر از {product.amount} باشد ",
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            ...
            if product.variant != "None":
                ...
                checkinvariant = ShopCart.objects.filter(
                    variant_id=variant.id, user_id=user.id
            ...
            if control == 1:
                if product.variant == "None":
                    instance = ShopCart.objects.get(
                        product_id=product.id, user_id=user.id
                    )
                else:
                    instance = ShopCart.objects.get(
                        product_id=product.id, variant_id=variant.id, user_id=user.id
                    )
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            selected_address = UserAddress.objects.get(
                user__username=current_user, selected=True
            )
            ...
            if shopcart:
                ...
                ).delete()  # Clear & Delete shopcart
                serializer = OrderDetailSerializer1(data, context={"request": request})
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            else:
                return Response(
                    data="ShopCart is empty", status=status.HTTP_201_CREATED
                )

...
from account.models import UserProfile, UserAddress, History
...
class UserRegisterSerializer(serializers.ModelSerializer):
    re_password = serializers.CharField(
        style={"input_type": "password"}, write_only=True, label="تکرار گذرواژه"
    )
...
class UserSerializer(serializers.ModelSerializer):
    ...
    national_code = serializers.CharField(
        source="profile.national_code", allow_null=True, required=False
    )
    image = serializers.ImageField(
        source="profile.image", allow_null=True, default="users/image/avatar.png"
    )
...
class UserAddressListSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        other_address = UserAddress.objects.filter(user_id=user).exclude(
            id=new_address.id
        )
        if other_address:
...
class OrderProductListSerializer(serializers.ModelSerializer):
    ...
    product = ProductSerializerLink1(
        many=False,
        read_only=True,
    )
...
class OrderDetailSerializer(serializers.ModelSerializer):
    user = serializers.CharField()
    products_set = OrderProductListSerializer(
        source="order",
        many=True,
        read_only=True,
    )
...
class ProfileCommentListSerializer(serializers.ModelSerializer):
    comment_url = serializers.HyperlinkedIdentityField(
        view_name="profile_comment_delete_api"
    )
